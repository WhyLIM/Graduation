 ...
 # @Timestamp for Last Modification: 2022-04-24 16:51:34
 # @Description: This R file is used to integrate the target-drug mapping relationships.
 #  Some drugs from TTD is removed, as they were repeated with the drugs in DrugBank.

library(tidyverse)
setwd("D:/Study/Project/Graduation/6_target_module/")

rm(list = ls())
DB_info <- read.delim("../1-3_target_match/DrugBank_Target.csv", sep = ",", header = T, stringsAsFactors = F)
TTD_info <- read.delim("../1-3_target_match/TTD_Target.csv", sep = ",", header = T, stringsAsFactors = F)
module_label <- read.table("../0_prer_work/node_Module.txt", header = T)
corenet <- read.table("../0_prer_work/top10%_CoreNet.txt", header = F, stringsAsFactors = F)
...
# write.table(newnet, "mudule_DTN.txt", row.names = F, quote = F)
