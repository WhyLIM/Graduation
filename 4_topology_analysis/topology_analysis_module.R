 ...
 # @Timestamp for Last Modification: 2022-01-03 16:02:48
 # @Description: This R file is used to:
 # 1. Calculated the average topological parameters of target and non-target proteins
 # 2. Calculated the average topological parameters of all proteins in each module
 # 3. Plot the difference between the topological parameters of all modules
 # 4. Plot the difference between the topological parameters of the target module
 # 5. Plot the difference between the topological parameters of special modules and all other modules.

library(tidyverse)
library(ggplot2)
library(ggpubr)
library(do)
setwd("D:/Study/Project/Graduation/4_topology_analysis")

rm(list = ls())
topology <- read.delim("module_topology_parameters.csv", sep = ",", header = T, stringsAsFactors = F)
target0 <- read.csv("../1-3_target_match/coretarget.csv", header = T, stringsAsFactors = F)
non_target0 <- read.csv("../1-3_target_match/non-coretarget.csv", header = T, stringsAsFactors = F)
module_label <- read.table("../0_prer_work/node_module.txt", header = T, stringsAsFactors = F)
