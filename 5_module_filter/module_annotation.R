 ...
 # @Timestamp for Last Modification: 2022-01-18 23:52:46
 # @Description: This R file is used to: (1) count the proportion of target in each module; 
 # (2) calculate the degree of each module; (3) calculate the TFCs of each module.
...
library(ggplot2)
library(ggpubr)
setwd("D:/Study/Project/Graduation/5_module_filter")

rm(list = ls())
protein_type <- read.csv("../1-3_target_match/protein_type_module.csv", header = T)
...
TFCs <- read.table("../0_prer_work/WCN.txt", header = T, stringsAsFactors = F)
edge_module <- read.table("../0_prer_work/edge_Module.txt", header = T, stringsAsFactors = F)
