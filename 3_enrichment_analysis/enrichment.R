 ...
 # @Timestamp for Last Modification: 2022-02-15 14:02:58
 # @Description: This file is used to draw the enrichment analysis picture of target protein and non-target protein 
 # after dividing the module. If using the background gene set, load the DOSE package.

# library(DOSE)
library(clusterProfiler)
library(org.Hs.eg.db)
library(ggplot2)
setwd("D:/Study/Project/Graduation/3_enrichment_analysis")

rm(list = ls())
target_symbol <- read.table("../1-3_target_match/coretarget_module.csv", header = T, stringsAsFactors = F)
non_target_symbol <- read.table("../1-3_target_match/non-coretarget_module.csv", header = T, stringsAsFactors = F)
